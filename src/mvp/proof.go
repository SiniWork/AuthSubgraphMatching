...
func (t *Trie) AuthFilter(q *matching.QueryGraph) ([]Node, map[Node]bool) {
	/*
	...
	*/
	...
	return nodeList, nodeListB
}

func (t *Trie) AuthSearch(key []byte) ([]int, []Node, bool) {
	/*
	obtaining candidate vertex set and merkle proof for the given key (one query vertex)
	*/
	var vNodes []Node
	if len(key) == 0 {
		return nil, vNodes, false
	}
	var result []int
