...
func reSearch(key []byte, nodeList []mvp.Node, nodeListB map[mvp.Node]bool, CSG map[int][]int) []int {
	/*
	...
	*/
	...
	if root, ok := rootNode.(*mvp.BranchNode); ok {
		...
		for {
			...
			if leaf, ok := node.(*mvp.LeafNode); ok {
				//fmt.Println("leaf node")
				matched := mvp.PrefixMatchedLen(leaf.Path, key)
				if matched == len(key) || mvp.IsContain(leaf.Path[matched:], key[matched:]){
			...
			if branch, ok := node.(*mvp.BranchNode); ok {
				//fmt.Println("branch node")
			...
			if ext, ok := node.(*mvp.ExtensionNode); ok {
				//fmt.Println("extension node")
				matched := mvp.PrefixMatchedLen(ext.Path, key)
				if matched < len(ext.Path) && matched < len(key){
					if ext.Path[len(ext.Path)-1] < key[matched] {
						...
						containAll, i := mvp.ContainJudge(ext.Path[matched:], key[matched:])
						if containAll{
							key = []byte{}
							node = ext.Next
							continue
...
func checkBranch(key []byte, b mvp.Node, nodeListB map[mvp.Node]bool) (bool, []potentialPath) {
	/*
	Checking whether the branch that needs to be added is empty
	*/
	if branch, yes:= b.(*mvp.BranchNode); yes {
		var subBranches int8
		if len(key) == 0 {
			subBranches = mvp.BranchSize
		} else {
